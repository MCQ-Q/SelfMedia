...
导演位输出后必须进入确定性质量校验，不能只依赖 `qualityChecklist` 自评。

硬性校验：

- `segments` 按 `orderIndex/startTime` 连续排序，时间段不能重叠，首段必须从 0 开始。
- `durationSeconds` 与最后一段 `endTime` 偏差不得超过 1 秒。
- 每段必须包含 `dialogue`、`subtitleText`、`hook`、`emotion`、`visual`、`bgm`、`edit`。
- 字幕单行长度、口播字数/秒、段落时长必须符合 `target_platform` 和 `duration_seconds` 的配置。
- 图片/视频提示词必须包含画幅方向和主体描述，禁止只有抽象情绪词。
- 校验失败时先进行一次结构化 repair retry；仍失败则标记 `director` 步骤失败，保留各支线输出供用户重试。
...
| idempotency_key | varchar(120) | 创建请求幂等键，可为空 |
...
| status | varchar(20) | pending / running / success / failed / skipped / cancelled |
...
| parent_version_id | uuid FK nullable | 重写或派生来源版本 |
| version_no | int | 版本号 |
...
- `version_no` 在同一项目内递增且唯一。
- 初次台本生成的 `parent_version_id` 为空。
- 单段重写必须创建新 `script_version`，并通过 `parent_version_id` 指向被重写的旧版本，避免已导出的版本被静默覆盖。
- `change_reason` 记录 `segment_rewrite`、`full_regenerate` 或用户填写的重写反馈摘要。
...
| scope | varchar(80) | 幂等范围，如 `projects:create`、`workflow:script_generation` |
| idempotency_key | varchar(120) | 请求头传入的幂等键 |
| request_hash | varchar(128) | 规范化请求体哈希 |
| status | varchar(20) | processing / succeeded / failed |
| resource_type | varchar(40) | 已创建资源类型 |
| resource_id | uuid | 已创建资源 ID |
...
- 同一 `user_id + scope + idempotency_key` 只能存在一条记录。
- 相同 key 且 `request_hash` 一致时返回首次响应。
- 相同 key 但 `request_hash` 不一致时返回 `409 IDEMPOTENCY_CONFLICT`。
- `processing` 状态重复提交时返回已存在的 `resource_id` 或 `409 REQUEST_IN_PROGRESS`，避免重复创建工作流和重复调用 LLM。
...
CREATE UNIQUE INDEX uniq_active_workflow_per_project_type
ON workflow_runs(project_id, workflow_type)
WHERE status IN ('pending', 'running');

CREATE UNIQUE INDEX uniq_workflow_idempotency
ON workflow_runs(project_id, workflow_type, idempotency_key)
...
CREATE UNIQUE INDEX uniq_script_version_no_per_project
ON script_versions(project_id, version_no);
CREATE INDEX idx_script_segments_version ON script_segments(script_version_id, order_index);
CREATE UNIQUE INDEX uniq_script_segment_key_per_version
ON script_segments(script_version_id, segment_key);

CREATE INDEX idx_project_events_project_id ON project_events(project_id, id);

CREATE UNIQUE INDEX uniq_idempotency_key_scope
...
- `POST /projects`、`POST /topics/generate`、`POST /topics/regenerate`、`POST /references/search`、`POST /scripts/generate`、`POST /script-segments/:segmentId/rewrite` 必须处理 `Idempotency-Key`。
- 服务端按 `user_id + scope + Idempotency-Key` 写入 `idempotency_keys`，并保存规范化请求体的 `request_hash`。
- 同一 key、同一请求体重复提交时，返回首次创建的资源或首次成功响应。
- 同一 key、不同请求体重复提交时，返回 `409 IDEMPOTENCY_CONFLICT`。
- 同一项目同一 `workflow_type` 同一时间只允许一个 `pending/running` 工作流；如果已有活动工作流，返回 `409 WORKFLOW_ALREADY_RUNNING` 并带上 `workflowRunId`。
- 工作流进入 `success/failed/cancelled` 终态时，同步更新对应 `idempotency_keys.status` 和 `response_snapshot`。
...
- `exported/approved` 版本视为不可变；后续手动编辑或单段重写必须派生新的 `script_version`。
- `PATCH /script-segments/:segmentId` 只允许修改当前 `draft` 版本；如果目标段落属于 `exported/approved` 版本，返回 `409 SCRIPT_VERSION_IMMUTABLE`。
...
```ts
async function startTopicGeneration(projectId: string, userId: string, idempotencyKey?: string) {
  return workflowStartService.start({
    projectId,
    userId,
    workflowType: "topic_generation",
    input: {},
    runningProjectStatus: "material_input",
    runningStep: "topics",
    idempotencyKey,
    steps: [{ stepKey: "topic", dependsOn: [] }],
  });
}

async function startReferenceCollection(projectId: string, topicId: string, userId: string, idempotencyKey?: string) {
  return workflowStartService.start({
    projectId,
    userId,
    workflowType: "reference_collection",
    input: { topicId },
    runningProjectStatus: "topic_selected",
    runningStep: "references",
    idempotencyKey,
    steps: [{ stepKey: "reference", dependsOn: [] }],
  });
}

async function startScriptGeneration(projectId: string, topicId: string, userId: string, idempotencyKey?: string) {
  return workflowStartService.start({
    projectId,
    userId,
    workflowType: "script_generation",
    input: { topicId },
    runningProjectStatus: "script_generating",
    runningStep: "script",
    idempotencyKey,
    steps: [
...
const workflowStartService = {
  async start(spec: WorkflowStartSpec) {
    return db.transaction(async (tx) => {
      await assertProjectOwner(spec.projectId, spec.userId, tx);
      const replay = await idempotencyService.findReplay(spec, tx);
      if (replay) {
        return { workflowRun: replay.workflowRun, shouldDispatch: false };
      }

      await idempotencyService.claim(spec, tx);
      await workflowRepo.assertNoActiveWorkflow(spec.projectId, spec.workflowType, tx);

      const before = await projectRepo.getStatusForUpdate(spec.projectId, tx);
      const workflowRun = await workflowRepo.create({
        ...
        projectStatusBefore: before.status,
        currentStepBefore: before.currentStep,
      });

      await workflowRepo.createSteps(tx, workflowRun.id, spec.steps);
      await projectStateService.applyWorkflowStarted(tx, workflowRun, {
        status: spec.runningProjectStatus,
        currentStep: spec.runningStep,
      });
      await eventRepo.emit(tx, spec.projectId, workflowRun.id, "workflow_created", {});

      return { workflowRun, shouldDispatch: true };
    }).then(async ({ workflowRun, shouldDispatch }) => {
      if (shouldDispatch) {
...
- `workflowStartService.start` 是唯一创建工作流入口，负责权限、幂等、活动工作流互斥、保存 `project_status_before/current_step_before`、创建步骤、切换项目运行态。
- `workflowDispatcher.start` 只负责将 `workflow_runs.status` 从 `pending` 更新为 `running`，写入 `workflow_started` 事件，并触发第一次 `tick`。
- 所有工作流启动前都必须经过 `workflowStartService.start`，禁止在业务接口中手写 `workflowRepo.create + createSteps + dispatcher.start`。
...
- `segment_rewrite.segment_rewrite` 成功后基于当前版本创建新 `script_version`，只替换指定段落及其支线。
...
`claimPendingStep` 必须在数据库事务中执行，并同时确认所属工作流仍处于 `running`，语义等价于：
...
WHERE workflow_steps.id = $1
  AND workflow_steps.workflow_run_id = workflow_runs.id
...
- `workflowDispatcher.runAgent` 在真正调用 Agent 前必须再次读取 `workflow_runs.status`，如果已取消或失败，直接跳过并把已 claim 的 step 标记为 `skipped/cancelled`。
...
async function runAgentJob(job: AgentJob) {
  ...
  try {
    if (!await workflowRepo.isRunning(job.workflowRunId)) {
      await workflowRepo.markClaimedStepCancelled(job.workflowStepId);
      return;
    }

    context = await buildAgentContext(job.workflowRunId, job.agentType);
    agentRun = await agentRunRepo.createFromJob(job, context.model);
    ...
    const applied = await db.transaction(async (tx) => {
      await workflowRepo.assertRunningForUpdate(job.workflowRunId, tx);
    ...
    await db.transaction(async (tx) => {
      if (agentRun) {
        await agentRunRepo.markFailed(agentRun.id, summarized, tx);
      }
      await workflowRepo.markStepFailed(job.workflowStepId, summarized, tx);
    });

    const projectId = context?.projectId ?? await workflowRepo.findProjectId(job.workflowRunId);
    await eventRepo.emit(projectId, job.workflowRunId, "agent_failed", {
      agentType: job.agentType,
...
- `buildAgentContext`、`createFromJob`、`agent_started` 失败也必须把已 claim 的 `workflow_step` 标记为 `failed`，不能让步骤长期停留在 `running`。
- LLM 返回后、写入 `agent_outputs` 前必须在事务内再次确认 `workflow_runs.status = 'running'`；如果工作流已取消，丢弃输出并结束。
- `agent_failed` 事件允许 `runId = null`，用于覆盖 Agent 运行记录尚未创建就失败的场景。
...
async function applyAgentOutput(params: ApplyAgentOutputParams) {
  ...
  switch (agentType) {
    ...
    case "segment_rewrite": {
      const rewritten = await scriptRepo.createVersionFromSegmentRewrite(tx, {
        segmentId: context.segmentId,
        rewriteTargets: context.rewriteTargets,
        output,
      });
      return {
        scriptVersionId: rewritten.scriptVersionId,
        segmentId: rewritten.segmentId,
      };
    }
    default:
...
- `segment_rewrite` 输出写入新台本版本后，必须返回新的 `scriptVersionId` 和新段落 `segmentId`，供前端切换到新版本。

### 12.4 单段重写流程

MVP 阶段单段重写采用“基于当前版本派生新版本”的方式，不原地覆盖旧 `script_version`。
...
- 重写成功后，在事务中复制当前版本的全部 `script_segments` 和 `segment_tracks`，只替换目标段落及指定支线，生成新的 `script_version`。
- 新版本的 `parent_version_id` 指向旧版本，`change_reason` 保存用户反馈摘要。
- 重写完成后写入 `project_events`，事件 payload 必须包含新的 `scriptVersionId` 和新的 `segmentId`，前端切换到新版本并定位该段。

伪代码：

```ts
async function rewriteSegment(segmentId: string, input: RewriteInput, userId: string, idempotencyKey?: string) {
  ...
  return workflowStartService.start({
    ...
    input: {
      segmentId,
      sourceScriptVersionId: segment.scriptVersionId,
      rewriteTargets: input.rewriteTargets,
      feedback: input.feedback,
    },
    runningProjectStatus: segment.projectStatus,
    runningStep: "script",
    idempotencyKey,
    steps: [{ stepKey: "segment_rewrite", dependsOn: [] }],
  });
}
```
...
- Access Token 存 HttpOnly Cookie。
- Refresh Token 只存哈希。
- 用户原始素材、手动录入参考全文、Agent 输入上下文和可包含原文片段的 Agent 输出按敏感数据处理。
- `source_materials.content_encrypted` 必须加密存储；`references.transcript` 如果来自用户手动粘贴或包含第三方完整字幕，也必须加密或拆分为加密正文 + 脱敏预览。
- `agent_outputs.content` 默认不写入原始素材全文；如调试需要保存完整上下文，必须标记 `metadata.sensitivity = "private"` 并加密。
- MVP 暂不引入密钥版本管理，先通过单一 `ENCRYPTION_KEY` 跑通核心闭环。
- 日志、错误信息、SSE 事件和 `agent_runs.input_snapshot` 不得输出原始素材全文、完整参考字幕或完整 Prompt。
- 导出前提示用户确认敏感信息。
...
| 级别 | 范围 | 存储策略 | 可进入日志/SSE |
|---|---|---|---|
| private_raw | 用户聊天记录、手动粘贴全文、完整参考字幕 | 加密正文 + 脱敏预览 | 否 |
| private_derived | Agent 输出中包含的用户原句、可识别个人信息 | 尽量结构化抽取，必要时加密 | 仅摘要 |
| public_reference | 搜索结果标题、公开 URL、公开摘要 | 明文可存 | 可摘要 |
| operational | 状态、耗时、token、错误码 | 明文可存 | 可以 |
